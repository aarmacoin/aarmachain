...
func (rs Receipts) DeriveFields(config *params.ChainConfig, hash common.Hash, number uint64, txs Transactions) error {
	...
	for i := 0; i < len(rs); i++ {
		...
		rs[i].ValidatorReward = txs[i].ValidatorReward(rs[i].GasUsed)
		rs[i].FoundationReward = txs[i].FoundationReward(rs[i].GasUsed)
		rs[i].SystemReward = txs[i].SystemReward(rs[i].GasUsed)
