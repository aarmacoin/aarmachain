...
func (api *API) GetSnapshot(number *rpc.BlockNumber) (*Snapshot, error) {
	...
	return api.yantra.snapshot(api.chain, header.Number.Uint64(), header.Hash(), nil)
}
...
func (api *API) GetSigners(number *rpc.BlockNumber) ([]common.Address, error) {
	...
	snap, err := api.yantra.snapshot(api.chain, header.Number.Uint64(), header.Hash(), nil)
	if err != nil {
		return nil, err
	}
...
func (api *API) Proposals() map[common.Address]bool {
	api.yantra.lock.RLock()
	defer api.yantra.lock.RUnlock()

	proposals := make(map[common.Address]bool)
	for address, auth := range api.yantra.proposals {
...
func (api *API) Propose(address common.Address, auth bool) {
	api.yantra.lock.Lock()
	defer api.yantra.lock.Unlock()

	api.yantra.proposals[address] = auth
}
...
func (api *API) Discard(address common.Address) {
	...
	delete(api.yantra.proposals, address)
}
...
func (api *API) Status() (*status, error) {
	...
	for n := start; n < end; n++ {
		...
		sealer, err := api.yantra.Author(h)
		if err != nil {
			return nil, err
		}
		signStatus[sealer]++
...
// GetSigner returns the signer for a specific yantra block.
...
func (api *API) GetSigner(rlpOrBlockNr *blockNumberOrHashOrRLP) (common.Address, error) {
	if len(rlpOrBlockNr.RLP) == 0 {
		...
		return api.yantra.Author(header)
	}
	block := new(types.Block)
	if err := rlp.DecodeBytes(rlpOrBlockNr.RLP, block); err == nil {
		return api.yantra.Author(block.Header())
	}
